      ...
      <section id="operator" className="scroll-mt-4 space-y-4">
        ...
        <Steps
          steps={[
            ...
            ["차량 등록", "‘지구 차량’ → 차량 등록. 노선(상/하행)·출발·도착·정원·1인 요금을 입력하고, 다른 지구에 열어줄 ‘잔여 좌석’을 공개해요."],
            ["신청 받기", "다른 지구 학생/간사가 신청하면 그 차량 상세의 ‘대기 신청’ 큐에 떠요. (학생 본인 직접 신청은 ‘학생 직접 신청’ 배지로 구분돼요.)"],
            ["승인", "큐에서 태울 학생을 선택해 ‘승인’ → 매칭 생성(송금 대기). 잔여 좌석 안에서 부분 승인도 가능해요."],
        ...
        <div className="rounded-lg bg-amber-50 px-3 py-3 text-xs leading-relaxed text-amber-800">
          ...
          본인 지구 인원이 <b>아직 확정되지 않았어도 괜찮아요.</b> 좌석을 여유 있게 열어두고,
          신청이 들어오는 상황을 보면서 <b>‘잔여 좌석’을 조정</b>하면 돼요. (정원은 고정이고, 다른
          지구에 여는 좌석 수만 조절 — 우리 지구 인원이 늘면 줄이고, 남으면 더 열고.) 자리가 확정될
          때까지 기다리기보다 <b>먼저 여유 있게 열어두는 편</b>이 매칭이 잘 돼요.
        </div>
      ...
      <section id="student" className="scroll-mt-4 space-y-4">
        ...
        <Steps
          steps={[
            ...
            ["예약하기", "홈에서 ‘예약하기’ → 방향(상/하행)·날짜로 조회 → 지도·목록에서 차량 선택 → 본인 정보 확인·개인정보 동의 → 신청."],
