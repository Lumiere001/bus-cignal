...
- 📍 **CC 세션 (2026-06-05 오후 — ⭐ 여기부터 읽으세요)**: **팀장 영역 진행 — ① RLS 하드닝(옵션 A) 머지 대기 ② anonymize_after 보류(종료일 미확정) ③ PWA 검토만.** 열린 PR 0 유지(RLS는 미push 브랜치).
  - ✅ **1. RLS policies 실적용 → 옵션 A 채택**: 접근모델 분석 = 앱 전량 service_role(`createAdminClient` 46곳) 우회 + 커스텀 JWT(jose, `auth.uid()`=null)라 **DB RLS 실효 = anon키 PostgREST 직격 차단뿐이고 그건 이미 enable+deny-default(정책 0)로 성립.** 이를 *의도된 보안경계*로 확정+강화. **브랜치 `feat/rls-hardening-revoke`(로컬 커밋 `525e93c`, push 대기)**: ① 코드점검 통과(service_role 클라번들 누출 0·`NEXT_PUBLIC` 노출 0·`SUPABASE_SERVICE_ROLE_KEY`=`admin.ts` 단일지점·anon경로 1곳=`region_locations` 공개읽기) ② 하드닝 마이그 `20260605000002_rls_hardening_revoke.sql`(PII 11테이블 anon·authenticated GRANT revoke, 공개읽기 2개 제외, service_role 무영향) ③ 결정문 `docs/decisions/2026-06-05-rls-deny-default-boundary.md`. **옵션 B(DB레벨 지구별 RLS via Supabase JWT)=출시후 트랙 / C(Supabase Auth)=비채택.** ⚠️ **prod 적용·push는 팀장 게이트**(Docker 부재로 로컬 db reset 검증 보류 — REVOKE 멱등+테이블명 대조로 정적보장).
  - ⏸️ **2. anonymize_after = 보류** (수련회 종료일 미확정). SPEC §5.9의 `2026-08-10`은 대시보드 mockup placeholder(D-105가 오늘과 불일치)+CCC IT 대기 → 확정 외부날짜 없음. cron(`app/api/cron/anonymize`)은 미설정 시 `"미설정"` skip = **무해**. **확정 시 세팅 = `/admin/system` 마스터 폼 `setAnonymizeAfter` 이미 완비 → 익명화예정일=종료일+90일 입력(예 2026-11-08). 코드/배포 0, prod 직접쓰기라 Cowork/팀장 영역.**
  - 🔍 **3. PWA 검토만(미구현)**: manifest=`app/manifest.ts`(App Router), icons=`/icon.svg` 1개뿐(TODO 주석 이미 존재). FCM SW=`app/firebase-messaging-sw.js/route.ts` 동적서빙+옵트인 등록(`lib/push/client.ts`, 동작 OK). **아이콘 192/512**=maskable PNG 에셋만 받으면 manifest 몇 줄(≈10분, 출시전 권장·디자인 협업). **offline PWA(next-pwa)**=미설치 → **출시후 권장**(next16 App Router 호환·FCM SW와 2-SW 공존·동적데이터 캐시전략 리스크 vs 단일수련회 가치 낮음). 둘 다 출시 차단 아님.
  - 🔜 **다음**: 팀장 = `feat/rls-hardening-revoke` push·PR·머지 + prod 마이그 적용 / 수련회 종료일 확정 시 `/admin/system`에서 anonymize 세팅. 외부 대기 = CCC 인증. 팀원2 = Phase C 배너 `/me` 마운트. 팀원1 = `/admin` 모바일 nav.
