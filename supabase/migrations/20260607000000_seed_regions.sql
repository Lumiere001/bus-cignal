-- Seed canonical 지구 (regions) reference data.
--
-- WHY THIS MIGRATION EXISTS:
--   supabase/seed.sql only runs on a local `supabase db reset`, never on prod.
--   As a result production had an empty `regions` table → "지구 선택 안됨".
--   This migration runs as part of `supabase db push`, so production gets the
--   canonical 지구 list. It mirrors supabase/seed.sql exactly.
--
-- SAFETY:
--   - Pure DATA insert. No schema changes.
--   - Idempotent: `on conflict (code) do nothing` makes it safe to re-run and
--     safe even if some regions already exist (e.g. partially seeded prod).
--   - Reference data only (no PII).
--   - Runs after the regions table is created in 20260528000001_initial_schema.sql.
--
-- 53 regions total (regular + special_ministry + overseas, incl. 9999 해외).
-- Keep in sync with supabase/seed.sql (data/regions.csv 기반).

insert into regions (code, name, area, category) values
